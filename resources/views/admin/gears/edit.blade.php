...
<div class="card">
  ...
  <div class="card-body">
    ...
    <form action="{{ route(ADMIN.'.gears.update', $gear->id) }}" method="POST">
      @csrf
      @method("PUT")
      <div class="tab-content p-3 border" id="myTabContent">
        ...
        <div class="tab-pane fade {{ $lang_key==0  ? 'show active' : '' }}" id="{{ $lang->shortened }}-pane" role="tabpanel" aria-labelledby="{{ $lang->shortened }}" tabindex="0">
          <div class="row mb-3">
            ...
            <div class="col-sm-9">
              <input type="text" value="{{ $gear->translate($lang->shortened)->name }}" class="form-control" name="data[{{ $lang->shortened }}][name]" id="name"
          ...
          <div class="row mb-3">
            ...
            <div class="col-sm-9">
              <input type="text" value="{{ $gear->translate($lang->shortened)->slug }}" class="form-control" name="data[{{ $lang->shortened }}][slug]" id="slug"
        ...
        <div class="row mb-3">
          ...
          <div class="col-sm-9">
            <select name="status" class="js-example-basic-single form-select" id="status">
              ...
              <option value="1" {{ $gear->status ==1 ? 'selected' : '' }}>{{ __('active') }}</option>
              <option value="0" {{ $gear->status ==0 ? 'selected' : '' }}>{{ __('deactive') }}</option>
            </select>
          </div>
        </div>
        <div class="mb-3">
