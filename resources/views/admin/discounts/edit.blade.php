...
@section('content')
	...
	<div class="card-header">
      ...
      <div class="card-body">
        <form class="forms-sample" action="{{ route(ADMIN.'.discounts.update', $discount->id) }}" method="POST" enctype="multipart/form-data">
			@csrf
      @method('PUT')
          <div class="row mb-3" id="brand-section">
            ...
            <select name="brands_id" class="js-example-basic-single form-select brand-content" id="brands_id">
              ...
              @foreach ($brands as $brand)
                <option value="{{ $brand->id }}" {{ $discount->brands_id==$brand->id ? 'selected' : '' }}>{{ $brand->name }}</option>
              @endforeach
            </select>
            </div>
          </div>
          ...
          <div class="row mb-3" id="model-section">
            ...
            <select name="models_id" class="js-example-basic-single form-select model-content" id="models_id">
              ...
              @foreach ($models as $model)
                <option value="{{ $model->id }}" {{ $discount->models_id==$model->id ? 'selected' : '' }}>{{ $model->name }}</option>
              @endforeach
            </select>
            </div>
          </div>
          ...
          <div class="row mb-3" id="car-section">
            ...
            <select name="cars_id" class="js-example-basic-single form-select car-content" id="cars_id">
              <option selected disabled>Seçin</option>
              @foreach ($cars as $car)
                <option value="{{ $car->id }}" {{ $discount->cars_id==$car->id ? 'selected' : '' }}>{{ $car->brands->name }} {{ $car->models->name }} {{ $car->name }}</option>
              @endforeach
            </select>
            </div>
          </div>
          ...
          <div class="row mb-3" id="car-section">
            ...
            <div class="col-sm-5">
              <select name="type" class="js-example-basic-single form-select discount-type" id="type">
                <option selected disabled>Seçin</option>
                <option value="1" {{ $discount->type == 1 ? 'selected' : '' }}>Faizlə</option>
                <option value="0" {{ $discount->type == 0 ? 'selected' : '' }}>Manatla</option>
              </select>
            </div>
            <div class="col-sm-5">
              <input type="number" name="discount" value="{{ $discount->discount }}" class="form-control" id="discount" placeholder="Endirimi daxil edin">
            </div>
          </div>
          ...
          <div class="row mb-3" id="car-section">
            <label for="type" class="col-sm-2 col-form-label">Tarix aralığı</label>
              <div class="col-md-5">
                <input type="date" class="form-control" value="{{$discount->start_date }}" name="start_date" id="start_date">
              </div>
              <div class="col-sm-5">
                <input type="date" class="form-control" value="{{$discount->end_date }}" name="end_date" id="end_date">
              </div>
          </div>
          ...
          <div class="row mb-3">
            ...
            <div class="col-sm-10">
              <select name="status" class="js-example-basic-single form-select" id="status">
                <option selected disabled>Seçin</option>
                <option value="1" {{$discount->status == 1 ? 'selected' : '' }}>Aktiv</option>
                <option value="0" {{$discount->status == 0 ? 'selected' : '' }}>Deaktiv</option>
              </select>
            </div>
          </div>
