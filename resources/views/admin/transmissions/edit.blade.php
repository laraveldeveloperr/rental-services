...
<div class="card">
  ...
  <div class="card-body">
    ...
    <form action="{{ route(ADMIN.'.transmissions.update', $item->id) }}" method="POST">
      @csrf
      @method('PUT')
      <div class="tab-content p-3 border" id="myTabContent">
        ...
        <div class="tab-pane fade {{ $lang_key==0  ? 'show active' : '' }}" id="{{ $lang->shortened }}-pane" role="tabpanel" aria-labelledby="{{ $lang->shortened }}" tabindex="0">
          <div class="row mb-3">
            <label for="name" class="col-sm-3 col-form-label">{{ __('transmission') }}</label>
            <div class="col-sm-9">
              <input type="text" value="{{ $item->translate($lang->shortened)->name }}" class="form-control" name="data[{{ $lang->shortened }}][name]" id="name"
                placeholder="{{ __('enter_transmission') }}">
            </div>
          </div>
          ...
          <div class="row mb-3">
            ...
            <div class="col-sm-9">
              <input type="text" value="{{ $item->translate($lang->shortened)->slug }}" class="form-control" name="data[{{ $lang->shortened }}][slug]" id="slug"
        ...
        <div class="row mb-3">
          ...
          <div class="col-sm-9">
            <select name="status" class="js-example-basic-single form-select" id="status">
              ...
              <option value="1" {{ $item->status==1 ? 'selected' : '' }}>{{ __('active') }}</option>
              <option value="0" {{ $item->status==0 ? 'selected' : '' }}>{{ __('deactive') }}</option>
            </select>
          </div>
        </div>
        <div class="mb-3">
