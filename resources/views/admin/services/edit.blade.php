...
<div class="card">
  <div class="card-header">
    <h3>{{ $service->title }}
      <div class="float-end">
        <a class="btn btn-danger btn-xs" href="{{ route(ADMIN.'.services.index') }}">
          <span>
  ...
  <div class="card-body">
    ...
    <form class="forms-sample" action="{{ route(ADMIN.'.services.update', $service->id) }}" method="POST" enctype="multipart/form-data">
      @csrf
      @method("PUT")
      <div class="tab-content p-3 border" id="myTabContent">
        ...
        <div class="tab-pane fade {{ $lang_key==0  ? 'show active' : '' }}" id="{{ $lang->shortened }}-pane"
          ...
          <div class="row mb-3">
            ...
            <div class="col-sm-9">
              <input type="text" class="form-control" value="{{ $service->translate($lang->shortened)->title }}"
          ...
          <div class="row mb-3">
            ...
            <div class="col-sm-9">
              <input type="text" class="form-control" value="{{ $service->translate($lang->shortened)->slug }}"
          ...
          <div class="row mb-3">
            ...
            <div class="col-sm-9">
              <textarea id="summernote-{{ $lang->shortened }}" name="data[{{ $lang->shortened }}][content]"
                height="300">{{ $service->translate($lang->shortened)->content }}</textarea>
            </div>
          </div>
        </div>
        @endforeach
        ...
        <div class="row mb-3">
          ...
          <div class="col-sm-9">
            <input type="file" name="image" id="myDropify" data-default-file="{{ asset('images/blogs').'/'.$service->image }}" />
          </div>
        </div>
        ...
        <div class="row mb-3">
          ...
          <div class="col-sm-9">
            <select name="status" class="js-example-basic-single form-select" id="status">
              ...
              <option value="0" {{ $service->status == 0 ? 'selected' : '' }}>{{ __('deactive') }}</option>
            </select>
          </div>
        </div>

        <button type="submit" class="btn btn-primary me-2">{{ __('udpate') }}</button>
      </div>
    </form>
  </div>
</div>
