...
<div class="card">
  ...
  <div class="card-body">
    ...
    <form class="forms-sample" action="{{ route(ADMIN.'.slides.update', $slide->id) }}" method="POST" enctype="multipart/form-data">
      @csrf
      @method('PUT')
      <div class="tab-content p-3 border" id="myTabContent">
        ...
        <div class="tab-pane fade {{ $lang_key==0  ? 'show active' : '' }}" id="{{ $lang->shortened }}-pane"
          ...
          <div class="row mb-3">
            ...
            <div class="col-sm-9">
              <input type="text" class="form-control" value="{{ $slide->translate($lang->shortened)->title }}"
                name="data[{{ $lang->shortened }}][title]" id="title" placeholder="enter_title">
            </div>
          </div>
          ...
          <div class="row mb-3">
            ...
            <div class="col-sm-9">
              <textarea id="summernote-{{ $lang->shortened }}" name="data[{{ $lang->shortened }}][content]"
                height="300">{{ $slide->translate($lang->shortened)->content }}</textarea>
            </div>
          </div>
        </div>
        @endforeach
        ...
        <div class="row mb-3">
          ...
          <div class="col-sm-9">
            <input type="file" name="image" id="myDropify" data-default-file="{{ asset('images/slides').'/'.$slide->image }}" />
          </div>
        </div>
        ...
        <div class="row mb-3">
          ...
          <div class="col-sm-9">
            <select name="status" class="js-example-basic-single form-select" id="status">
              ...
              <option value="1" {{ $slide->status== 1 ? 'selected' : '' }}>{{ __('active') }}</option>
              <option value="0" {{ $slide->status== 0 ? 'selected' : '' }}>{{ __('deactive') }}</option>
            </select>
          </div>
        </div>
