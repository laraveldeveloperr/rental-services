...
<div class="card">
  ...
  <div class="card-body">
    ...
    <form class="forms-sample" action="{{ route(ADMIN.'.faqs.update', $element->id) }}" method="POST"
      ...
      <div class="tab-content p-3 border" id="myTabContent">
        ...
        <div class="tab-pane fade {{ $lang_key==0  ? 'show active' : '' }}" id="{{ $lang->shortened }}-pane"
          ...
          <div class="row mb-3">
            ...
            <div class="col-sm-9">
              <input type="text" class="form-control" value="{{ $element->translate($lang->shortened)->question }}"
          ...
          <div class="row mb-3">
            ...
            <div class="col-sm-9">
              <textarea id="summernote-{{ $lang->shortened }}" name="data[{{ $lang->shortened }}][answer]"
                height="300">{{ $element->translate($lang->shortened)->answer }}</textarea>
            </div>
          </div>
        </div>
        @endforeach
        ...
        <div class="row mb-3">
          ...
          <div class="col-sm-9">
            <select name="status" class="js-example-basic-single form-select" id="status">
              ...
              <option value="1" {{ $element->status == 1 ? 'selected' : '' }}>{{ __('active') }}</option>
              <option value="0" {{ $element->status == 0 ? 'selected' : '' }}>{{ __('deactive') }}</option>
            </select>
          </div>
        </div>
